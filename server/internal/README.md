...
| discord | Discord 频道工作区（adr-016）：gateway 长连接、/init 绑定表单、仓库克隆与配置存储（`<dataDir>/discord.json`）。**与会话零耦合**：项目内只 import gitrepo，模型清单由装配层经 CatalogFunc 注入，哨兵错误自带（writeError 里同义映射） | 业务 |
