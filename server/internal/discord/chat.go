...
// cronInstructions 是定时任务运行的无人值守约定：追加在会话提示词后
// （claude），也原样进每次运行的开场输入（两端都生效）。写给一个刚打开、
// 没有人陪着的会话看——它最容易犯的错是停下来问，以及交一份「计划」。
const cronInstructions = `# 无人值守运行须知

这一轮是定时任务在没有人在线的情况下自动运行的：

- 不要提问、不要等待确认——没有人会回答。拿不准就按最保守的口径做，并在结论里注明假设。
- 最终回复必须是**成品**（结论 + 成果），不是计划、不是「我将…」。
- 避开需要人工审批的操作：写文件用 Write/Edit，图表手写内联 SVG，不跑脚本；数据库 / 服务器 / 报告 / 交付这些只读工具面都是预批的，优先用它们。
- 如果这是巡检类任务且没有任何值得汇报的内容，整条回复只写 NO_REPORT 这一个词。但工具报错、连不上机器、查询超时这类失败要如实汇报——那不是「无事」。
- 数据从工具里来，没查到的数字不写。`
