...
- 之后直接在子区里聊，不用再 @；手动创建的子区同样有效。子区里 @ 同事说的话 AI 不插嘴，要它也听见就把它一起 @ 上
...
- **定时任务**：在子区里说「以后每天早上 10 点出一份发到这个频道」，AI 会建一条定时任务；到点频道里自动开子区跑，跑完能在子区里追问；`+"`/cron`"+` 看与管
- **排队与编辑**：回合进行中发的消息标 ⏳ 排队、下一轮自动带上；还标着 ⏳ 的消息可以编辑
- **权限与提问**：agent 请求授权或提问时出卡片——点按钮、填表单，或直接回复编号
- **停下来**：回合一开始子区里就有一张过程卡，点上面的 **⏹ 停止** 掐掉当前回合并清空排队；`+"`/stop`"+` 等效

## 看代码改动
- `+"`/git`"+` 列出本频道工作树的分支、与远端的差距，以及改了 / 新增 / 删除了哪些文件

## 看懂状态
⏳ 已排队 · ✅ 已进对话；📋 计划卡与 🔧 过程卡实时刷新（过程卡上能停）；回合结束回复末尾有小结（耗时 · 工具 · 改动文件 · token）
