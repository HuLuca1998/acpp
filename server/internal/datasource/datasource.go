...
// SSH 隧道的验证方式（照 Navicat 的三选一）。定义在 sshdial，
// 这里只保留别名——本包与前端契约里到处在用这三个字面量。
const (
	SSHAuthPassword = sshdial.AuthPassword
	SSHAuthKey      = sshdial.AuthKey
	SSHAuthBoth     = sshdial.AuthBoth
)
...
// Servers 是服务器配置的最小依赖：按 id 取一台机器。开了 SSH 隧道的
// 数据源要靠它拿到跳板机的地址与凭证（adr-019 把这些搬进了服务器表）。
//
// 与 Sessions / Calls 同理用接口而不是直接 import remote 包——两个业务包
// 因此不互相 import。为 nil 时隧道拨不出去（会报「没有关联跳板机」），
// 不开隧道的数据源不受影响。
type Servers interface {
	ServerByID(ctx context.Context, id uint) (*model.Server, error)
}
...
// WithServers 挂上服务器配置的来源。分开一个 setter 而不是塞进 NewService：
// 装配顺序上服务器面与数据源面互不依赖，谁先构造都行。
func (s *Service) WithServers(servers Servers) *Service {
	s.servers = servers
	return s
}
...
func (s *Service) ProbeSSH(ctx context.Context, id uint, in Input) (string, error) {
	...
	if serverID == 0 {
		return "", fmt.Errorf("%w: 先选一台跳板机再测试", service.ErrInvalid)
	}
	if s.servers == nil {
		return "", fmt.Errorf("%w: 服务器配置面未装配", service.ErrInvalid)
	}
	srv, err := s.servers.ServerByID(ctx, serverID)
	if err != nil {
		return "", err
	}
...
func decorate(src *model.DataSource) {
	...
	if src.Server != nil {
		src.ServerName = src.Server.Name
	}
}

// finish 给出库的记录补上跳板机配置与派生字段。**所有读取路径都要走它**
// ——漏掉的那条路上，开着隧道的数据源会在拨号时报「没有关联跳板机」。
//
// 服务器被删掉时 Server 留空而不是报错：数据源本身没坏，只是隧道拨不出去
// ——那个错该在真的要连的时候报（connect 里说得很具体），而不是让列表页
// 整个 500。数据源是个位数量级，没有为 N+1 做批量取的必要。
func (s *Service) finish(ctx context.Context, list ...*model.DataSource) {
	for _, src := range list {
		if s.servers != nil && src.ServerID > 0 {
			srv, err := s.servers.ServerByID(ctx, src.ServerID)
			if err != nil {
				slog.Warn("数据源关联的服务器取不到",
					"datasource", src.ID, "server", src.ServerID, "err", err)
			}
			src.Server = srv
		}
		decorate(src)
