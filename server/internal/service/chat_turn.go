...
//
// 引用**只以 resource 块下发**（URI + 一段「怎么查」的告知，见
// datasource 包的 expandRef），绝不能给 prompt 加裸 text 块：转录重建时
// 所有 text 块都会被拼成用户正文（emitUserBlocks），注入的说明会显示成
// 用户自己说的话。早先的用法约定就是这么漏进用户气泡的——rebuild.go 里
// 至今留着对那段旧文案的兼容跳过（legacyDBGuidance）。
//
