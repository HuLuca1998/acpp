...
- **列表页统一四区，不自己排版**：页头 `ListPageHeader`（标题 + 条数）之下就是一个 `DataTable`，它自上而下画搜索区（`search`：`SearchBar` 包 `SearchText` / `SearchSelect`，状态用 `useSearchDraft`，提交才请求、提交与重置都回第一页）→ 操作区（`actions` 在左，刷新 `onReload` 与列显隐在右；传 `fetching` 让进度线与转圈联动）→ 表格区（一张卡片，没行时在同一张卡里画 `ListPageStates`）→ 翻页。新建按钮属于操作区，筛选控件属于搜索区，都不放页头；页面外层统一 `flex flex-col gap-4 p-4 lg:p-6`。不再把整页包进 `Card`——卡片只给表格区，六个列表页的边距才会对齐。
