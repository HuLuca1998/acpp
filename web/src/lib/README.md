...
| desktop.ts        | 桌面壳（macOS app）的原生通道：环境判定、启动偏好、系统通知（授权/发送/撤回），走 Electron 壳 preload 注入的消息口而非 HTTP；桌面版自更新的阶段判定 | `isDesktop`、`desktopLaunch`、`desktopNotify`、`NotifyStatus`、`NOTIFICATION_ACTION_EVENT`、`isUpdateActive` |
...
| use-panel-visible.ts | dockview 面板可见性 + 「可见才加载/刷新」（隐藏 tab 标脏、切回补拉）                                            | `usePanelVisible`、`useVisibleLoad`            |
| use-workspace-watch.ts | 工作目录文件变动流（SSE）订阅：节流回调、页面进后台即断、后端说不监视就不重连 | `useWorkspaceWatch`                            |
