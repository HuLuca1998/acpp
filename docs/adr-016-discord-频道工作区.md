...
- **单卡原则**：一个频道永远只有一张字段化的「工作区身份卡」（自动置顶），
  所有操作都改它——/init 重绑继承卡 id、/model /effort 原地刷新、解绑连卡
  删掉。/init 的过程反馈（分支选择、进行中、失败原因）全部走 ephemeral，
  不进频道时间线；置顶动作产生的系统消息也顺手清掉。频道主题另写一行摘要
  （`acpp 工作区：仓库 @ 分支 · 模型 · 思考深度`）。主题接口限速狠（每频道
  10 分钟 2 次，实测 429 retry_after 可达 5 分钟），撞了按 retry_after 挂
  延迟重写（取最新状态，连续变更收敛成一次）——置顶卡才是权威展示。
  卡被人删了，下次配置变更会补发一张再置顶。
