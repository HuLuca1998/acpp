...
- **菜单栏图标**：左键切换主窗口显隐；右键菜单：**我的 issue**（分配给我的 issue 清单，条件与 GitHub 页默认一致——open、排除做完 / 取消的看板列、按优先级排，最多 15 条，点一条打开 GitHub；「查看全部 issue」进 `/github`；「用哪个 Chrome 账号打开」选一个 Chrome profile，私有仓库只有某个账号有权限时用，直接跑 Chrome 二进制传 `--profile-directory` 才不会被已运行的实例忽略）/ 打开主窗口 / 在浏览器中打开 / 允许局域网访问 / 复制局域网链接 / 开机启动 / 开机最小化 / 重启服务 / 打开服务日志 / 退出。issue 清单壳里缓存、每 3 分钟刷新、弹菜单时顺手再拉一次。
