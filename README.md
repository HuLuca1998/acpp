...
- 每轮结束后由重建器**按时序**还原成多条消息：正文被工具调用打断处就是断点（agent「说一句 → 干点活 → 再说一句」会还原成两条消息，而不是首尾相接的一条），同一段内的连续分片仍然合并。前端收到 `turn_done` 后重新拉取消息列表，用重建结果取代流式拼接的文本，所以偶尔丢一个 chunk 也不会留下残缺消息。
