...
- **菜单栏图标**：左键只弹 **我的 issue** 清单（照 codex-ui 的 Codex Viewer，图标就是一张 issue 清单），右键（或 ⌃+左键）弹操作菜单。issue 清单是分配给我的 issue（条件与 GitHub 页默认一致——open、排除做完 / 取消的看板列、按优先级排，最多 15 条，**点一条复制纯数字编号**，⌥ 点击才打开 GitHub；「查看全部 issue」进 `/github`；「刷新 issue」带 `refresh=1` 让后端立刻去 GitHub 拉一次；「用哪个 Chrome 账号打开」选一个 Chrome profile，私有仓库只有某个账号有权限时用，直接跑 Chrome 二进制传 `--profile-directory` 才不会被已运行的实例忽略）。操作菜单：打开主窗口 / 在浏览器中打开 / 允许局域网访问 / 复制局域网链接 / 开机启动 / 开机最小化 / 重启服务 / 打开服务日志 / 退出。issue 清单壳里缓存、每 3 分钟刷新、弹菜单时顺手再拉一次（这两种都只读后端缓存，后端自己每 3 分钟刷一轮 GitHub）。
