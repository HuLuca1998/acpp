...
├── desktop/           # macOS 桌面壳（electron/ Electron 主进程、preload 与 popover/ 托盘面板页面；icons/ 图标绘制脚本），选型见 docs/adr-015，打包走 scripts/build-macos-app.sh
...
- 现有自建 skill：[skills-manage](.claude/skills/skills-manage/SKILL.md)——acpp 技能库的管理操作与 SKILL.md 写作规范，涉及技能管理功能（页面、API、注入）时先读它；[acp-debug](.claude/skills/acp-debug/SKILL.md)——ACP 协议排障手册（转录读法、probe、故障排查表）；[discord-e2e](.claude/skills/discord-e2e/SKILL.md)——Discord 真机测试操作手册（建删测试频道、驱动斜杠命令与组件的实测手法、从后端取证、回归清单与收尾清理）。Codex 都可直接当文档读。
